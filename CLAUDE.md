...
- Cases live in `/cases/{slug}/` — never delete a case directory manually
- AI-internal files (interrogation.md, debate.md) are stored in `~/.gcbc/cases/{slug}/` — outside the project
...
- This applies to the Chief's topic assignment, GC/BC question generation, investigation prompts, and direct conversations.

## Interrogation Rounds

- Each round, a **Chief Investigator** first assigns topics to GC and BC — no overlap
- Both GC and BC ask questions every round, but on different topics
- The Chief can reassign topics across rounds (e.g., a weak GC answer gets handed to BC next round)
- Minimum 4 rounds before sufficiency can be signaled (each persona needs depth across rounds)
